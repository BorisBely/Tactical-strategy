...
16. [Исправления v2.2 — единый скоринг и тирированные проверки](#17-исправления-v22--единый-скоринг-и-тирированные-проверки)
...
| `Severity` | FeasibilitySeverity | **v2.2** — градация: Valid/Risky/Unsafe/Impossible |
...
| Уровень | Смысл | Когда |
|---------|-------|------|
| `Valid` | Безопасно | Все проверки пройдены |
| `Risky` | Рискованно, но можно | Узкий проход, небольшой недобор зазора |
| `Unsafe` | Нежелательно | Зазор < порога, но не критично |
| `Impossible` | Невозможно | Обрыв, зазор < 50% порога |

Фабрики: `Valid`, `Risky(risk, reason)`, `Unsafe(reason)`, `Impossible(reason)`, `Invalid(reason)` (старая = Unsafe).

### 6.2 ManeuverFeasibilityChecker
`Assets\_Scripts\Vehicle\Navigation\ManeuverFeasibilityChecker.cs`

Тирированные пороги (v2.2):
- **Forward:** < 0.9м → Impossible, < 1.8м → Unsafe, узкий → Risky
- **TurnAround:** < 0.35R → Impossible, < 0.7R → Unsafe
- **Drop:** двойной луч (оба должны промазать) → Impossible

### 6.3 VehicleLocalGeometry (v2.2 — двойной луч)
`Assets\_Scripts\Vehicle\Navigation\VehicleLocalGeometry.cs`

Проверка обрыва: два луча вперёд (0.5м и 1.5м от носа), оба на 5м вниз. Только если ОБА не попали в землю → `HasDropAhead = true`. Устраняет ложные срабатывания на неровной местности.
...
| ��� | ������� | ����������� |
|-----|---------|-------------|
| ������ � ����� ����� ���� ����� � �������� ���������� | Fallback ������ ���� Forward ���������� �� ��������� | Fallback ����� � ����� ������ �� Score. Impossible=+999999 ����������� ��� Forward ��������� TurnAround |
| ������ �� ������ � �������� | TurnAround �������� 0.8?R ������ �������, ������ ����� | ����������: <0.35R > Impossible, <0.7R > Unsafe (��������� ��� fallback) |
| ������ �drop ahead� �� �������� ��������� | ���� ��� �� ������� � ����� �� ������� | ������� ���: ��� ������ ���������. ��������� ��������� � 3� �� 5� |
| ���������� Pursuit �� ������� ������ | 2 waypoints �� 20+ ������ | ������������� ����� ������ 5� ��� ����� >10� |
| ����� ������ ����� ���������� | Clearance < 1.8� > Invalid | ����������: <0.9� > Impossible, <1.8� > Unsafe |
...
�������: Valid, Risky(risk, reason), Unsafe(reason), Impossible(reason).
...
ApplyRiskPenalty() ��������� ����� �� Severity:
- Impossible > +999999
- Unsafe > +200
- Risky > +50
- Valid > +0

�������������: HasCliffRisk +50, HasNarrowPassage +10.

### 17.4 ������� ��� ������

� VehicleLocalGeometry.Probe() ��� ���� ����� (0.5� � 1.5� �� ����), ��� �� 5� ����. ������ ���� ��� �� ������ � ����� > HasDropAhead = true. ���������� ��� HasDropBehind.

��� ��������� ������ ������������ �� �������� ���������, ��� ���� ��� ����� �� ������� � ground ��-�� �������� �����.

### 17.5 ������������� waypoints

PathPlanner.BuildDirectPath() � ��� ������ fallback > 10� ��������� ������������� ����� ������ 5�. Pure Pursuit �������� �� ������� ������ ��� ����������.

### 17.6 ���������� �����

| ���� | ��� ���������� |
|------|---------------|
| FeasibilityResult.cs | + FeasibilitySeverity enum, ������� Impossible/Unsafe/Risky |
| ManeuverFeasibilityChecker.cs | ������������ ������: Forward 0.5R/1.0R, TurnAround 0.35R/0.7R |
| ScoringSystem.cs | ApplyRiskPenalty ��������� SeverityPenalty ������ MaxValue |
| DrivingPlanner.cs | Fallback �����, ����� ������ �� Score |
| VehicleLocalGeometry.cs | ������� ��� ��� �������� ������ |
| PathPlanner.cs | ������������� waypoints ��� ������ fallback >10� |
