...
namespace CombatVehicleSystem
{
	...
	public class WheeledMotor : MonoBehaviour
	{
		...
		private VehicleTireCompliance m_TireCompliance;
		...
		private void Awake()
		{
			...
			TryGetComponent(out m_TireCompliance);
			BindAntiStuck();
		...
		public void TickPhysics(bool _controlActive, VehicleCommand _command)
		{
			...
			// Soft tire carcass before visual sync (GetGroundHit only, no extra raycasts).
			if (m_Body != null && !m_Body.isKinematic && m_TireCompliance != null)
				m_TireCompliance.TickForces(m_Axles);
		...
		private void SyncVisuals()
		{
			...
			for (int i = 0; i < m_Axles.Length; i++)
			{
				...
				if (m_TireCompliance != null && m_TireCompliance.TryGetVisualWorldOffset(i, out Vector3 tireOffset))
					pos += tireOffset;
