...
public sealed class UnitAnimatorStance : MonoBehaviour
{
	...
	{
		if (!LocomotionProneFeature.Enabled && m_Stance == LocomotionStance.Prone)
		{
			StopReadyBeforeProneHack();
			m_PendingProne = false;
			m_PendingStandFromProne = false;
	...
	private void Update()
	{
		...
		if (IsLocomotionFullyStopped())
		{
			if (!LocomotionProneFeature.Enabled)
			{
				if (m_PendingProne)
					m_PendingProne = false;
				if (m_PendingStandFromProne || m_PendingCrouchFromProne)
				{
					m_PendingStandFromProne = false;
					m_PendingCrouchFromProne = false;
				}
			...
			if (LocomotionProneFeature.Enabled && m_PendingProne && m_Stance != LocomotionStance.Prone)
			{
				m_PendingProne = false;
				m_Stance = LocomotionStance.Prone;
				PushStance();
			...
			if (LocomotionProneFeature.Enabled && m_PendingStandFromProne && m_Stance == LocomotionStance.Prone)
			{
				m_PendingStandFromProne = false;
				m_Stance = LocomotionStance.Standing;
				PushStance();
			...
			if (LocomotionProneFeature.Enabled && m_PendingCrouchFromProne && m_Stance == LocomotionStance.Prone)
			{
				m_PendingCrouchFromProne = false;
				m_Stance = LocomotionStance.Crouch;
				PushStance();
		...
		{
			if (LocomotionProneFeature.Enabled && zPressed)
			{
				if (!IsLocomotionFullyStopped())
				{
					m_PendingProne = true;
