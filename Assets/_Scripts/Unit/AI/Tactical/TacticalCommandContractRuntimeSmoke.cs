...
/// <summary>
/// Stage 6.1 game command Play. Issues Attack → arrival → Defense → Retreat via IssueCommand.
/// Does not retune Vision / Combat / navigation executors. Source is this smoke, not RTS.
/// Report: Assets/_Docs/Logs/Tests/TacticalCommandContract_LAST.txt
/// </summary>
[DefaultExecutionOrder(62)]
[DisallowMultipleComponent]
[RequireComponent(typeof(DetectionTestController))]
public sealed class TacticalCommandContractRuntimeSmoke : MonoBehaviour
{
	#region Constants
	...
	public bool WillRunOnStart =>
		(m_RunOnStart || DetectionHarnessPlayMode.RunTacticalCommandContract) &&
	...
	private void Start()
	{
		...
		Debug.Log("[TacticalCommandContractRuntimeSmoke] Stage 6.1 starting.", this);
	...
	private IEnumerator RunSuite()
	{
		...
		m_PassCount = 0;
		m_FailCount = 0;
		AppendLine("STAGE 6.1 — TACTICAL GAME COMMAND CONTRACT");
		AppendLine("==========================================");
		AppendLine($"stamp={DateTime.Now:yyyy-MM-dd HH:mm:ss}");
		AppendLine("IssueCommand Attack → arrival stay Attack → Defense stop → Retreat. No RTS.");
		AppendLine("---");
		...
		yield return null;
		yield return RunAttackArrivalDefenseRetreat();
	...
	private IEnumerator RunAttackArrivalDefenseRetreat()
	{
		AppendLine("---");
		AppendLine("[C1] Idle → IssueCommand Attack(P) → Walk reason=Attack");
		ResetForOrder();
		Vector3 attackDest = SampleAway(20f);
		TacticalCommandResult attack = m_Controller.IssueCommand(
			TacticalCommand.Attack(attackDest, null, TacticalCommandSource.Scenario));
		Check("C1_Accepted", attack.Accepted, attack.Reason.ToString());
		Check("C1_Attack", m_Controller.CurrentState == UnitAIState.Attack, m_Controller.CurrentState.ToString());
		Check("C1_Dest", Approximately(m_Controller.CurrentContext.Destination, attackDest),
		...
		Check("C1_Walked", moved,
			$"start={startDist:F2} now={UnitSearchNavigationMath.PlanarDistance(m_Observer.position, attackDest):F2}");

		AppendLine("[C2] Arrival → HardStop, remains Attack");
		WarpObserver(attackDest);
		m_Controller.Tick(c_SimDt);
		yield return null;
		Check("C2_Reached", m_Controller.TacticalDestinationReached, "not reached");
		Check("C2_StillAttack", m_Controller.CurrentState == UnitAIState.Attack, m_Controller.CurrentState.ToString());
		Check("C2_Stopped", !m_Controller.SearchHasMoveIntent, $"intent={m_Controller.SearchHasMoveIntent}");

		AppendLine("[C3] IssueCommand Defense → Defense, nav stop");
		Vector3 hold = m_Observer.position;
		TacticalCommandResult defense = m_Controller.IssueCommand(
			TacticalCommand.Defense(hold, TacticalCommandSource.Scenario));
		Check("C3_Accepted", defense.Accepted, defense.Reason.ToString());
		Check("C3_Defense", m_Controller.CurrentState == UnitAIState.Defense, m_Controller.CurrentState.ToString());
		Check("C3_Anchor", Approximately(m_Controller.CurrentContext.AnchorPosition, hold),
			m_Controller.CurrentContext.AnchorPosition.ToString());
		yield return null;
		Check("C3_NavStop", !m_Controller.SearchHasMoveIntent, $"intent={m_Controller.SearchHasMoveIntent}");
		Check("C3_ReasonNone",
			m_Controller.CurrentNavigationReason == UnitNavigationReason.None,
			m_Controller.CurrentNavigationReason.ToString());

		AppendLine("[C4] IssueCommand Retreat(P2) → old Walk cancelled, MOVE reason=Retreat");
		Vector3 retreatDest = SampleAway(20f, Vector3.left);
		TacticalCommandResult retreat = m_Controller.IssueCommand(
			TacticalCommand.Retreat(retreatDest, TacticalCommandSource.Scenario));
		Check("C4_Accepted", retreat.Accepted, retreat.Reason.ToString());
		Check("C4_Retreat", m_Controller.CurrentState == UnitAIState.Retreat, m_Controller.CurrentState.ToString());
		Check("C4_Dest", Approximately(m_Controller.CurrentContext.Destination, retreatDest),
			m_Controller.CurrentContext.Destination.ToString());
		Check("C4_NotAttackDest",
			UnitSearchNavigationMath.PlanarDistance(retreatDest, attackDest) > 1f,
			$"A={attackDest} B={retreatDest}");
		yield return null;
		Check("C4_ReasonRetreat",
	...
	private void ResetForOrder()
	{
		...
		m_Controller.IssueCommand(TacticalCommand.Cancel(TacticalCommandSource.Scenario));
	}
	...
	private void Finish()
	{
		...
		string path = Path.Combine(dir, "TacticalCommandContract_LAST.txt");
		File.WriteAllText(path, m_Report.ToString(), Encoding.UTF8);
		Debug.Log(
			$"[TacticalCommandContractRuntimeSmoke] wrote {path} " +
			$"RESULT={(m_FailCount == 0 ? "PASS" : "FAIL")} pass={m_PassCount} fail={m_FailCount}",
			this);

		bool exitPlay = m_ExitPlayModeWhenDone || DetectionHarnessPlayMode.RunTacticalCommandContract;
#if UNITY_EDITOR
	...
	private void Check(string _name, bool _ok, string _detail)
	{
		...
		else
		{
			...
			Debug.LogError($"[TacticalCommandContractRuntimeSmoke] FAIL {_name} | {_detail}", this);
		}
	}
