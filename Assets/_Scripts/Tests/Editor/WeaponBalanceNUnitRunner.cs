...
{
	/// <summary>
	/// Phase G G-TEST-1..10 EditMode fixtures without Test Runner window selection.
	/// </summary>
	public static class WeaponBalanceNUnitRunner
	{
		...
		public static void RunFromMenu()
		{
			EditModeNUnitRun.Execute(
				new Filter
				{
					...
					groupNames = new[] { "Shooting.Tests.WeaponBalanceRunnerTests" }
				},
				new Callbacks());
			Debug.Log(
				"[WeaponBalance G-TEST] EditMode started: WeaponBalanceRunnerTests (G-TEST-1..10). " +
		...
		private sealed class Callbacks : ICallbacks
		{
			...
			{
				Debug.Log(
					$"[WeaponBalance G-TEST] finished passed={result.PassCount} failed={result.FailCount} skipped={result.SkipCount}");
			...
			public void TestFinished(ITestResultAdaptor result)
			{
				...
				Debug.LogError($"[WeaponBalance G-TEST] FAIL {result.Test.FullName}: {result.Message}");
			}
		}
	}
}
